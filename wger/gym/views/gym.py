...
class GymListView(LoginRequiredMixin,
...
class GymUserListView(LoginRequiredMixin,
    ...
    def get_context_data(self, **kwargs):
        ...
        context['gym'] = Gym.objects.get(
            pk=self.kwargs['pk'])
        context['admin_count'] = len(
            context['object_list']['admins'])
        context['user_count'] = len(
...
def gym_new_user_info_export(request):
    ...
    filename = 'User-data-{t.year}-{t.month:02d}-{t.day:02d}-{user}.\
    csv'.format(t=today,
                user=new_username)
...
def gym_permissions_user_edit(request, user_pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            member.groups.remove(Group.objects.get(
                name='gym_member'))
            member.groups.remove(Group.objects.get(
                name='gym_trainer'))
            member.groups.remove(Group.objects.get(
            ...
            if 'user' in form.cleaned_data['role']:
                member.groups.add(
                    Group.objects.get(name='gym_member'))
            if 'trainer' in form.cleaned_data['role']:
                member.groups.add(
                    Group.objects.get(name='gym_trainer'))
            if 'admin' in form.cleaned_data['role']:
                member.groups.add(
                    Group.objects.get(name='gym_manager'))
...
class GymAddUserView(WgerFormMixin,
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        return super(GymAddUserView, self).dispatch(
            request, *args, **kwargs)
    ...
    def get_context_data(self, **kwargs):
        ...
        '''
        context = super(GymAddUserView, self).\
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if request.user.has_perm('gym.manage_gym')\
            ...
            if request.user.userprofile.gym_id != int(
                    self.kwargs['pk']):
                return HttpResponseForbidden()
        return super(GymUpdateView, self).dispatch(
            request, *args, **kwargs)
...
class GymDeleteView(WgerDeleteMixin, LoginRequiredMixin,
    ...
    def get_context_data(self, **kwargs):
        ...
        Send some additional data to the template
        '''
        context = super(GymDeleteView, self).\
            get_context_data(**kwargs)
