...
class WorkoutManagerTestCase(BaseTestCase, TestCase):
    ...
    def post_test_hook(self):
        '''
        Hook to add some more specific tests
        after the basic add or delete
        operations are finished
        '''
        pass


class WorkoutManagerDeleteTestCase(
        WorkoutManagerTestCase):
    '''
    Tests deleting an object an authorized user,
    a different one and a logged out
    one. This assumes the delete action is
    only triggered with a POST request and
...
class WorkoutManagerAccessTestCase(WorkoutManagerTestCase):
    '''
    Tests accessing a URL per GET as an
    authorized user, an unauthorized one and
    a logged out one.
    '''
