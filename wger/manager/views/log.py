        ...
        formset = WorkoutLogFormSet(
                ...
                initial=[
                    {'weight_unit': user_weight_unit,
                     'repetition_unit': 1} for x in range(0,
                                                          total_sets)])

        dateform = HelperDateForm(initial={'date': datetime.date.today()})

        # Depending on whether there is already a
        # workout session for today, update the current one or create
        # a new one (this will be the most usual case)
        if WorkoutSession.objects.filter(
                user=request.user,
